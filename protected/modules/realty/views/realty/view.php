...
/**
 * @todo Так делать грешно! Отрефакторить!
 */
$this->title = "Просмотр: " . Realty::model()->getRealtyType($model->type) . ', ' . $model->addressTable->getAddress();
?>

<?php
$additionalInfo = ($model->realtyDetailed)?$model->realtyDetailed->viewAdditional():array();
//echo CVarDumper::dump($additionalInfo,10,true);exit;
?>
...
'attributes'=> CMap::mergeArray(array(
    'type:realtyCategory',
    array(
        'label' => Yii::t("RealtyModule.realty", 'Агент по недвижимости'),
        'value' => function($data) {
    ...
    array(
        'label' => Yii::t("RealtyModule.realty", 'Вид недвижимости'),
        'value' => function($data) {
            if (isset($data->realtyDetailed->type)) {
                return RealtyDetailedDescription::model()->previewSubtype($data->type, $data->realtyDetailed->type);
            }

            return 'N/A';
        },
        'visible'=>isset($data->realtyDetailed->type)
    ),
    array(
