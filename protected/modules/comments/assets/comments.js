        ...
        return this.each(function(){
                ...
                if(confirm($.fn.commentsList.settings[id]['deleteConfirmString']))
                {
                    ...
                    .success(function(data){
                        ...
                        if(data["code"] === "success")
                        {
                            ...
                            $('.blog-details-left-top .count-comments-blog').text(data["count"]);
                        }
                    });
                }
                return false;
            ...
            .delegate('.add-comment', 'click', function(){
                ...
                if(commentID){
                    ...
                    $('html, body').animate({
                        scrollTop: $("#blog-details-form").offset().top
                    }, 2000);
                }
...
function sendComment(event){
    ...
    $.post(
        ...
        data = $.parseJSON(data);
        if(data["code"] == "success"){
            $form.find('#Comment_user_name, #Comment_user_email, #Comment_comment_text').val('');
            var id = $('.comment-widget').attr('id');
            $('#'+id).html($(data["list"]).html());
        }else {
            $('#ajax-errors').html($(data["form"]).html()).addClass('comment-errors');
        }
    });
}