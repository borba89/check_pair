<?php
    $realty = Realty::model()->findByPk($post['realty_id']);
    ...
    $tCriteria->params[':item_realty'] = $realty->id;
    $tCriteria->params[':item_offer'] = $post['temp_id'];
    $images = MultipleImages::model()->findAll($tCriteria);

    $additionalInfo = $realty->realtyDetailed->viewAdditional();
...
<div class="main">
    ...
    <?php $this->widget('main.widgets.PreviewSliderWidget', array(
        'images' => $images,
        'post' => $post,
        'realty' => $realty
    )); ?>
        ...
        <div class="article flex space-between mb-3em no-wrap-sm wrap-xs pb-1_5em">
            <div class="pl-1_5em-sm col-sm-12 col-md-8 pt-2em-xs">
                <div class="size-1_2em p-1em pt-2em pr-3em-lg">
                    <p><?php echo $realty->description; ?></p>
                </div>
            </div>
            <?php if($videos):?>
