...
class SeoController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','addmetaname',"deletemetaname","addmetaproperty","deletemetaproperty",'create','update','admin','delete','autocompletelayingproduct','autocompletelayingcategory'),
    ...
    */
    ...
    public function actionAutocompletelayingproduct()
        {
            Yii::app()->language = 'ru';
            $laings = Product::model()->findAll();
            $allLinks = array();
            foreach($laings as $laing)
            {
                $allLinks[] = $laing->url();
            }
            ...
            if(!empty($allLinks))
            {
                ...
                {
                    $isset_url = YiiseoUrl::model()->findByAttributes(array("url"=>$link['url']));
                    if(!$isset_url){
                        $model=new YiiseoUrl;
                        //$model->attributes=$_POST['YiiseoUrl'];
                        $model->url = $link['url'];
                        ...
                        if($model->save())
                        {
                            ...
                            if(isset($link['name']))
                            {
                                $items = array('title', 'description','keywords');
                                foreach($items as $key => $item)
                                {
                                    ...
                                    $mod->url = $model->id;
                                    if($key == 0)
                                        $mod->content = '3dplemya - Купить '.$link['name'];
                                    elseif($key == 1)
                                        $mod->content = 'Новейшие технологии на сайте 3dplemya - 3д принтеры, 3д сканеры и аксессуары к ним. Купить '.$link['name'];
                                    elseif($key == 2)
                                        $mod->content = 'Купить '.$link['name'];
                                    else
                                        $mod->content = $link['name'];
    ...
    public function actionAutocompletelayingcategory()
    {
        Yii::app()->language = 'ru';
        $laings = Category::model()->findAll();
