...
class BackendLoginForm extends CFormModel
{
    ...
    /**
     * Разрешить доступ к панели управления определенным ролям
     * @return bool
     */
    public function isStaffAndNotSuspended()
    {
        if ($user = User::model()->find('LOWER(email)=?', array($this->email))) {
            if (!in_array($user->role, User::allowedBackendRoles())) {
                $this->addError('password', 'Incorrect email or password.');
                return false;
            }
