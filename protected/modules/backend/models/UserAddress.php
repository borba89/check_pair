...
class UserAddress extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, user_id, type, address_street, city, state, postcode, country', 'required'),
			array('id, user_id', 'numerical', 'integerOnly'=>true),
			array('type', 'length', 'max'=>12),
			array('address_street', 'length', 'max'=>255),
			array('city, state, postcode, country', 'length', 'max'=>80),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, type, address_street, city, state, postcode, country', 'safe', 'on'=>'search'),
		);
	}
