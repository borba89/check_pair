...
class DToggleColumn extends CGridColumn
{
    ...
    protected function registerClientScript()
    {
        ...
        if(isset($this->aToggle))
            $toggle = "$(th).closest('a').toggleClass('$this->aToggle');";
        else
            $toggle = '';
    ...
    protected function renderDataCellContent($row,$data)
    {
        ...
        if ($visible)
        {
            ...
            if($this->onImageHtml)
                $onImage = $this->onImageHtml;

            if($this->offImageHtml)
                $offImage = $this->offImageHtml;

            if(isset($this->aTagOn) && isset($this->aTagOff))
                $aTagOptions = $value ? $this->aTagOn.' ' : $this->aTagOff.' ';
            else
                $aTagOptions = '';

            if (!empty($url))
                echo CHtml::link($onImage . $offImage, $url, array('class'=>'toggle-link '. $aTagOptions . $this->class . ($value ? ' toggle-true' : '')));
            else
