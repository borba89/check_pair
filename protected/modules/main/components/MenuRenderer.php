...
class MenuRenderer extends CMenu
{
    ...
    public function init() {
        ...
        if ($this->name) {
            $menu = MMenu::model()->findByAttributes(array('name' => $this->name));
        } elseif ($this->id) {
            $menu = MMenu::model()->findByPk($this->id);
        } elseif ($menu = MMenu::model()->findByAttributes(array('name' => 'Main'))) {
            //NOP, assignment done within the codition above
        } else {//find the first one
            $menu = MMenu::model()->find();
        }
