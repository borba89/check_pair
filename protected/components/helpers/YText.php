 ...
 *
 */
Yii::import('application.vendor.2amigos.transliterator-helper.src.dosamigos.yii.helpers.TransliteratorHelper');
...
class YText
{
     ...
     */
    public static function characterLimiter($str, $n = 500, $strip_html = true, $end_char = '&#8230;')
    {
        if ($strip_html)
            $str = strip_tags($str);
     ...
     *
     * @access public
     * @param  $str string - строка для обрезания
     * @param  $limit integer - до скольких символов обрезать строку
     * @param  $strip_html boolean - удалть html элементы
     * @param  $end_char string - окончание строки
     * @return string - новая строка
     */

    public static function wordLimiter($str, $limit = 100, $strip_html = true, $end_char = '&#8230;')
    {
        if ($strip_html)
            $str = strip_tags($str);
